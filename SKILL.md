...
Пользователь хочет создать или улучшить инструкцию для LLM. Это может быть system prompt, role prompt, agent instruction, prompt template, Claude Project setup, или конкретный промпт для повторяемой задачи. Скилл следует процессу: понять задачу → определить тип → применить шаблон → проверить по чеклисту → выдать результат.
